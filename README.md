...
└── 四页面：今日状态 / 限制规则 / 添加应用（含正在运行程序选择器）/ 设置
...
第三阶段（管理界面）已完成：服务直接托管 Vue 3 管理界面（四页面 + 进程选择器 + 7/30 天统计图 + PIN 门控）、本机 JSON API、ProductName 自动读取、CLI 增加 `usage --days` 与 `debug lock`。API 已 curl 全流程冒烟（含 PIN 鉴权与 Host 防护），界面已在浏览器中实测（状态页/添加页/进程选择器/PIN 弹窗均正常渲染与提交）。待办：托盘程序（计划第三阶段剩余项）。

后续阶段（见计划书）：托盘程序、防绕过加固（第四阶段）。
