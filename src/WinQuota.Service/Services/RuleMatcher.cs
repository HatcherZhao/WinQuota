...
/// <summary>
/// 把扫描到的进程与应用规则做匹配。四种条件取并集（任一命中即算匹配）：
/// 1. 完整路径精确匹配（配置 ExePath 时，防不同目录同名程序误伤）；
/// 2. 进程名匹配（廉价字符串比较）；
/// 3. 产品级匹配（ProductName / Publisher，防用户重命名或复制 exe 绕过）；
/// 4. 签名者匹配（数字签名 CN，需 WinVerifyTrust 校验通过，防文件被篡改后冒名）。
/// 路径、产品与签名信息仅在对应条件配置时才按需解析，控制每轮扫描开销。
/// </summary>
        ...
        IReadOnlyList<ApplicationRule> apps)
    {
        ...
        foreach (var snapshot in snapshots)
        {
            foreach (var app in apps)
            {
                ...
                // 签名者匹配（仅在规则配置了 Signer 时才做 WinVerifyTrust 校验）
                if (!string.IsNullOrWhiteSpace(app.Signer) &&
                    AppMatcher.MatchesBySignature(scanner.GetSignatureInfo(snapshot.Pid, snapshot.ProcessName), app))
                {
                    matched.Add(snapshot);
                    break;
                }
